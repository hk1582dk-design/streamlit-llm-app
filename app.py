...
def generate_response(input_text, role_selection, api_key):
    ...
    # 2. OpenAI SDKを使ってAPIリクエストを行う
    openai.api_key = api_key

    # 3. メッセージリストの作成（ChatCompletion API 形式）
    messages = [
    ...
    # エラーハンドリング：入力がない場合
        ...
        with st.spinner("AI専門家が回答を考えています..."):
            try:
                ...
                # 必要なパッケージはこのファイル内でインストールしないでください。
                # ターミナルで以下を実行して依存関係をインストールしてください:
                # pip install streamlit langchain-openai langchain
